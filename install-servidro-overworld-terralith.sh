...
SOURCE="$PROJECT_ROOT/content/worldgen/zz_servidro_overworld_terralith"
TARGET="$SERVER_DIR/$WORLD_NAME/datapacks/zz_servidro_overworld_terralith"

if [[ ! -d "$SOURCE" ]]; then
  echo "No existe $SOURCE. Genera el datapack de integracion primero."
  exit 1
fi

mkdir -p "$TARGET"
rsync -a --delete "$SOURCE"/ "$TARGET"/
echo "Integracion Servidro/Terralith instalada en $TARGET"
